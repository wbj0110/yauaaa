...
public class UserAgentAnalyzerDirect implements Analyzer, Serializable {
    ...
    public void loadResources(String resourceString) {
        ...
        if (resources.isEmpty()) {
            ...
            if (DEFAULT_RESOURCES.equals(resourceString)) {
               ...
               // PackagedRules.getRuleFileNames().forEach(this::loadResources);
                for(String fileName: PackagedRules.getRuleFileNames() ){
       ...
       Set<String> resourceBasenames =  resources.keySet();
     Iterator<String> it = resourceBasenames.iterator();
     while(it.hasNext()){
         it.next().replaceAll("^.*/", "");
     }
      ...
      //  Set<String> alreadyLoadedResourceBasenames = matcherConfigs
        ...
        Set<String> alreadyLoadedResourceBasenames =  matcherConfigs.keySet();
        Iterator<String> itB = resourceBasenames.iterator();
        while(itB.hasNext()){
            itB.next().replaceAll("^.*/", "");
        }
...
*/
        List<MappingNode> matcherConfigList = null;
        if(!matcherConfigs.containsKey(filename)){
    ...
    public void lookingForRange(String treeName, Range range) {
        ...
        if(!informMatcherActionRanges.containsKey(treeName)){
            ranges = new HashSet<>(4);
            informMatcherActionRanges.put(treeName,ranges);
        }
        ranges.add(range);
    }
    ...
    public void informMeAboutPrefix(MatcherAction matcherAction, String treeName, String prefix) {
        ...
        Set<Integer> lengths = null;
        if(!informMatcherActionPrefixesLengths.containsKey(treeName))
            lengths =  new HashSet<>(4);
    ...
    public void informMeAbout(MatcherAction matcherAction, String keyPattern) {
       ...
       /* Set<MatcherAction> analyzerSet = informMatcherActions
        ...
        Set<MatcherAction> analyzerSet = null;
            if(!informMatcherActions.containsKey(hashKey)){
    ...
    public Set<Range> getRequiredInformRanges(String treeName) {
       // return informMatcherActionRanges.computeIfAbsent(treeName, k -> Collections.emptySet());
        Set<Range> sets = null;
        if(!informMatcherActionRanges.containsKey(treeName)){
            sets = new HashSet<Range>();
            informMatcherActionRanges.put(treeName,sets);
        }
