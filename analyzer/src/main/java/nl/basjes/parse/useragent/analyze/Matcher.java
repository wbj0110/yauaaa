    ...
    public void initialize() {
        try {
           // variableActions.forEach(MatcherAction::initialize);
            for(MatcherVariableAction variablaction : variableActions){
                variablaction.initialize();
            }
      ...
      //  fixedStringActions.forEach(action -> dynamicActions.remove(action));
       ...
       // uselessRequireActions.forEach(action -> dynamicActions.remove(action));
        ...
        for(MatcherAction action : uselessRequireActions){
            dynamicActions.remove(action);
        }
...
*/
      ...
      if(!informMatcherActionsAboutVariables.containsKey(variableName)){
          ...
          informMatcherActionsAboutVariables.put(variableName,analyzerSet);
      }
